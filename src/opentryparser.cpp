...
void OpEntryParser::parseOpcodesXml(QString opcodeFile) {
    ...
    QString xmlData = QString::fromUtf8(file.readAll());
    file.close();

    QString errorMsg;
    int errorLine = 0;
    int errorColumn = 0;
    if (!m_doc.setContent(xmlData, false, &errorMsg, &errorLine, &errorColumn)) {
        // risset (csound's package manager) can generate a malformed opcodes.xml:
        // an <opcode> with no syntaxes is left unclosed (missing </opcode>), which
        // makes the whole file unparseable and would discard every risset opcode.
        qDebug() << "OpEntryParser::parseOpcodesXml could not parse opcode file:" << opcodeFile
                 << "at line" << errorLine << "column" << errorColumn << ":" << errorMsg;
        qDebug() << "OpEntryParser::parseOpcodesXml trying to repair malformed xml";
        xmlData = repairOpcodesXml(xmlData);
        if (!m_doc.setContent(xmlData, false, &errorMsg, &errorLine, &errorColumn)) {
            qDebug() << "OpEntryParser::parseOpcodesXml could not parse opcode file even after repair:"
                     << opcodeFile << "line" << errorLine << ":" << errorMsg;
            return;
        }
    }
    excludedOpcodes << "|" << "||" << "^" << "+" << "*" << "-" << "/";
...
QString OpEntryParser::repairOpcodesXml(const QString &xml)
{
    // CsoundQt parses opcodes.xml at startup, including the extra file generated by
    // risset (csound's package manager). risset can emit malformed xml: an <opcode>
    // element whose manpage has no syntaxes is left unclosed (missing </opcode>).
    // QDomDocument rejects the whole document on such an error, so every risset
    // opcode would silently lose syntax highlighting. This closes any <opcode>
    // element that is not properly closed before the next <opcode>, </category> or
    // </opcodes>, so the document becomes well-formed.
    QString result;
    result.reserve(xml.size());
    int openOpcodes = 0;
    int pos = 0;
    QRegularExpression tagRx("<[^>]+>");
    QRegularExpressionMatchIterator it = tagRx.globalMatch(xml);
    while (it.hasNext()) {
        QRegularExpressionMatch match = it.next();
        result += xml.mid(pos, match.capturedStart() - pos);
        pos = match.capturedEnd();
        QString tag = match.captured(0);
        const bool isOpcodeStart = tag.startsWith("<opcode") &&
                (tag.size() == 8 ? tag.at(7) == QLatin1Char('>')
                                 : tag.size() > 8 && tag.at(7) == QLatin1Char(' '));
        if (isOpcodeStart) {
            while (openOpcodes > 0) {
                result += "</opcode>";
        ...
        else if (tag == "</category>" || tag == "</opcodes>") {
            while (openOpcodes > 0) {
                result += "</opcode>";
                --openOpcodes;
            }
...
bool OpEntryParser::getOpcodeArgNames(Node &node)
{
    ...
    QStringList args = inArgs.split(QRegularExpression("[,\\\"]+"), SKIP_EMPTY_PARTS);
    for (int count = 0; count < args.size(); count ++) {
        args[count] = args[count].trimmed();
        if (args[count] == "")
            args.removeAt(count);
    }
    QStringList argsOpt = inArgsOpt.split(QRegularExpression("[,\\\\\\s\\[\\]]+"), SKIP_EMPTY_PARTS);
    ...
    args = outArgs.split(QRegularExpression("[,\\s]+"), SKIP_EMPTY_PARTS);
    argsOpt = outArgsOpt.split(QRegularExpression("[,\\\\\\s\\[\\]]"), SKIP_EMPTY_PARTS);

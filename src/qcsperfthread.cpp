...
class CsoundPerformanceThreadMessage {
 ...
 public:
    ...
    virtual int run() = 0;
    CsoundPerformanceThreadMessage(QCsPerfThread *pt)
    {
      pt_ = pt;
...
class CsPerfThreadMsg_Play : public CsoundPerformanceThreadMessage {
 public:
    CsPerfThreadMsg_Play(QCsPerfThread *pt)
...
class CsPerfThreadMsg_Pause : public CsoundPerformanceThreadMessage {
 public:
    CsPerfThreadMsg_Pause(QCsPerfThread *pt)
    : CsoundPerformanceThreadMessage(pt)
    {
    }
    int run()
...
class CsPerfThreadMsg_TogglePause : public CsoundPerformanceThreadMessage {
 public:
    CsPerfThreadMsg_TogglePause(QCsPerfThread *pt)
    : CsoundPerformanceThreadMessage(pt)
    {
    }
    int run()
...
class CsPerfThreadMsg_Stop : public CsoundPerformanceThreadMessage {
 public:
    CsPerfThreadMsg_Stop(QCsPerfThread *pt)
    : CsoundPerformanceThreadMessage(pt)
    {
    }
    int run()
...
class CsPerfThreadMsg_ScoreEvent : public CsoundPerformanceThreadMessage {
 private:
    ...
    MYFLT   p[10];
 public:
    CsPerfThreadMsg_ScoreEvent(QCsPerfThread *pt,
...
class CsPerfThreadMsg_InputMessage : public CsoundPerformanceThreadMessage {
 private:
    ...
    char    s[128];
 public:
    CsPerfThreadMsg_InputMessage(QCsPerfThread *pt, const char *s)
    : CsoundPerformanceThreadMessage(pt)
    {
      len = (int) strlen(s);
      if (len < 128)
...
class CsPerfThreadMsg_SetScoreOffsetSeconds
 ...
 private:
    double  timeVal;
 public:
    CsPerfThreadMsg_SetScoreOffsetSeconds(QCsPerfThread *pt,
                                          double timeVal)
    : CsoundPerformanceThreadMessage(pt)
    {
 ...
 * QCsPerfThread::Stop(), or an error occurs.
...
QCsPerfThread::QCsPerfThread(Csound *csound)
{
...
void QCsPerfThread::QueueMessage(CsoundPerformanceThreadMessage *msg)
{
    if (status) {
      delete msg;
      return;
...
void QCsPerfThread::TogglePause()
{
...
void QCsPerfThread::ScoreEvent(int absp2mode, char opcod,
                                         int pcnt, const MYFLT *p)
{
...
void QCsPerfThread::InputMessage(const char *s)
{
...
void QCsPerfThread::SetScoreOffsetSeconds(double timeVal)
{
...
void QCsPerfThread::FlushMessageQueue()
{
    if (firstMessage) {

...
<bsbPanel>
 ...
 <bsbObject version="2" type="BSBLabel">
  ...
  <midicc>0</midicc>
  <label>-------------------------------------------------------------------------------------------------------
reverb is a basic reverberation unit. Within the opcode an algorithm consisting of four parallel comb filters and two series allpass filters is employed Its only obligatory input argument besides the input audio signal is for reverb time in seconds. An optional 2-way switch defines the initial disposition of the delay loop data (0=cleared, 1=previous data retained). It should be bourne in mind that the 'reverb' algorithm represents quite a basic reverb algorithm and that with transient input audio granularity in the reverb effect becomes quite evident. (0=no filtering, 1=more filtering) In this example a Dry/Wet Mix crossfader is also implemented.

...
instr	1	;GUI
	...
	if (ktrig == 1)	then
		...
		kq			invalue 	"Resonance"
		gkq			tablei	kq, giExp500, 1
					outvalue	"Resonance_Value", gkq
...
<bsbPanel>
 ...
 <bsbObject version="2" type="BSBLabel">
  ...
  <midicc>0</midicc>
  <label>----------------------------------------------------------------------------------------------
Lowpass2 is an implementation of a second-order resonant IIR low-pass filter. Its input arguments are for filter cutoff frequency (in hertz), Q or resonance (a value in the range 1 to 500) and an optional 2-way switch (default=0) to select whether the data space for feedback within the filter is initially cleared or allowed to retain previously held information. Use of high levels of resonance can cause significant increases in the overall output amplitude. It may be desired to make use of the balance opcode to control these fluctuations in amplitude. Alternatively a control to scale the output amplitude can be implemented (as has been done in this example).
</label>

...
giExp1	ftgen	0, 0, 129, -25, 0, 20.0, 128, 10000.0		;TABLE FOR EXP SLIDER
...
;GUI
	...
	if (ktrig == 1)	then
		...
		kfr			invalue	"Frequency"
		gkfr			tablei	kfr, giExp1, 1
					outvalue	"Frequency_Value", gkfr
		gkfdbgain		invalue	"Feedback"
...
instr	2
	...
	UPDATE:
	aresL 		streson	gasigL, gkfr, i(gkfdbgain)
	aresR 		streson	gasigR, gkfr, i(gkfdbgain)
				rireturn									;RETURN FROM REINITIALISATION PASS TO PERFORMANCE TIME PASSES
				outs		aresL * gkamp, aresR * gkamp			;SEND streson OUTPUT TO THE AUDIO OUTPUTS AND SCALE USING THE GUI SLIDER VARIABLE gkamp
...
<bsbPanel>
 ...
 <bsbObject version="2" type="BSBLabel">
  ...
  <midicc>0</midicc>
  <label>-------------------------------------------------------------------------------------------------------------------------
Streson implements a sympathetic string resonating-type filter using comb, lowpass and allpass filter in an algorithm similar to that of the Karplus-Strong plucked string model. Streson's audible output is similar to that of the 'wguide1' opcode. One difference is that 'Feedback' is variable only at i-rate in streson. 'Feedback' is expressed as a ratio of the entire signal. A value of 0 means that none of the signal will be fed back into the system, a value of 1 means that all of the signal will be fed back into the system. The use of feedback can create dramatic rises in output amplitude, therefore in this example I have included a slider that controls output amplitude.
Note that tuning inaccuracies are increasingly exhibited as 'Frequency' is increased. If precise tuning is required then it will probably not be suffucient to rely on the value given for 'Frequency' and instead this should be checked by ear. In this example the user can choose between a sound file input, the computer's live input or a stream of click impulses. The risk of feedback is greatly increased through the use of streson processing so for this reason a gain control on the live input, which is initially zero, is included. Use with caution! It might actually be better to monitor through headphones to further reduce the risk of feedback when using the live input.</label>
  <alignment>left</alignment>
  <font>Liberation Sans</font>
 ...
 <bsbObject version="2" type="BSBLabel">
  ...
  <midichan>0</midichan>
  <midicc>0</midicc>
  <label>Feedback Gain (i-rate)</label>
  <alignment>left</alignment>
  <font>Liberation Sans</font>
  <fontsize>10</fontsize>
  <precision>3</precision>

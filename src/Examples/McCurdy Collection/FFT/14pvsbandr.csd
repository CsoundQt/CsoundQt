...
i 10		0	   3600	;GUI
 ...
 <bsbObject version="2" type="BSBLabel">
  ...
  <midicc>0</midicc>
  <label>----------------------------------------------------------------------------
pvsbandr is an implementation of a band reject filter using phase vocoder analysis.
...
Using this system the user can continuously vary the cutoff slope of the filter.
The input parameter 'type' define the shape of the cutoff slope. A value of zero will give a linear shape, positive values will result in an exponential shape and negative values will give a logarithmic shape.
The user can select between a variety of combinations of parameters for the FFT analysis. Larger values for FFT size will give better harmonic reproduction but greater time smearing effects and higher computational cost. Lower values will result in less time smearing but greater harmonic distortion.
The user can choose between different input sources which includes the computer's live input.</label>

...
{
    // Called by csound when the 'outvalue' opcode is used, on the performance thread.
    // Keep this minimal: no QString, no logging, no widget access. Just copy the
    // raw name and value into the widget layout's lock free queue; the GUI thread
    // applies it at the next update (WidgetLayout::processCsoundUpdates()).
    CsoundUserData *ud = (CsoundUserData *) csoundGetHostData(csound);
    const char vartype = ((CS_TYPE *) channelType)->varTypeName[0];
    if (vartype == 'S' || vartype == 's') {
        ud->wl->enqueueOutputString(channelName, (const char *) channelValuePtr);
    }
    else if (vartype == 'k' || vartype == 'K') {
        ud->wl->enqueueOutputValue(channelName, *((MYFLT *) channelValuePtr));
    }
}
                                       ...
                                       const void *channelType)
{
    ...
    CsoundUserData *ud = (CsoundUserData *) csoundGetHostData(csound);
    char vartype = csChannelType->varTypeName[0];
    ...
    else if ( vartype == 'k' || vartype == 'K') {  // Not a string channel
...
{
    // This is called every cycle, sends values from csoundqt to csound
    MYFLT* pvalue;
                ...
                *pvalue = i.value();
                ...
                if (csoundGetChannelPtr(ud->csound, (void **)&pvalue, i.key().toLocal8Bit().constData(),
                    ...
                    *pvalue = (MYFLT) i.value();
                    ud->channelPointers.insert(i.key(), pvalue);
                }
    ...
    if (!ud->wl->newStringValues.isEmpty() && ud->wl->stringValueMutex.tryLock(1)) {
...
{
    // This is called at every cycle during performance,
    // sends values from csound to csoundqt
    // NB: during a performance there are no changes in the widgetlayout (wl)
    // and also no changes in outputChannelNames or displayChannelNames, etc.
    MYFLT* pvalue;
    // QSet<QString>seenChannels;
    for(int i = 0; i < ud->outputChannelNames.size(); i++) {
        const QString &name= ud->outputChannelNames[i];
        if(name == "")
            continue;
        auto ptr = ud->channelPointers.constFind(name);
        if(ptr != ud->channelPointers.end()) {
            pvalue = ptr.value();
            MYFLT value = *pvalue;
            if(ud->previousOutputValues[i] != value) {
                ud->wl->setValue(name, value);
                ud->previousOutputValues[i] = value;
            }
        } else if(csoundGetChannelPtr(ud->csound, (void**)&pvalue, name.toLocal8Bit().constData(),
                                   CSOUND_OUTPUT_CHANNEL | CSOUND_CONTROL_CHANNEL) == 0) {
            ud->wl->setValue(name, *pvalue);
            ud->previousOutputValues[i] = *pvalue;
            ud->channelPointers.insert(name, pvalue);
        }
        // seenChannels.insert(name);
    }
    
    // for (int i = 0; i < ud->outputChannelNames.size(); i++) {
    //     const QString &name = ud->outputChannelNames[i];
    //     if (name != "" && !seenChannels.contains(name)
...
void CsoundEngine::setWidgetLayout(WidgetLayout *wl)
{
    ...
    // User facing messages generated by widgets (e.g. unknown property) are
    // delivered to the registered consoles with their explicit role (colour).
    connect(wl, &WidgetLayout::logMessage, this, &CsoundEngine::logWidgetMessage);
}
...
void CsoundEngine::setupCallbacks()
{
...
void CsoundEngine::setupChannels()
{
    ...
    foreach (QuteWidget *w, widgets) {
        ...
        if(channel != "") {
            if (ud->wl->channelNameToWidgets.contains(channel)) {
                ud->wl->channelNameToWidgets[channel].append(w);
            } 
            else {
                ud->wl->channelNameToWidgets.insert(channel, {w});
            }    
        }        
        
        if(channel2 != "") {
            if (ud->wl->channelNameToWidgets.contains(channel2)) {
                ud->wl->channelNameToWidgets[channel2].append(w);
            }
            else {
                ud->wl->channelNameToWidgets.insert(channel2, {w});
            }        
        }
        ...
        if (wtype == QuteWidgetType::DISPLAY || wtype == QuteWidgetType::TABLEDISPLAY) {
            ...
            int chantype = csoundGetChannelPtr(ud->csound, (void **) &pvalue, channel.toLocal8Bit().constData(), 0);
            bool channelExists = chantype >= 0;
            if(channelExists) {
                ...
                int res = csoundGetChannelPtr(ud->csound, (void **) &pvalue,
                                    channel.toLocal8Bit().constData(),
                                    CSOUND_INPUT_CHANNEL | CSOUND_OUTPUT_CHANNEL
                                    | CSOUND_CONTROL_CHANNEL);
                if(res >= 0) {
            ...
            if(!seenChannelNames.contains(channel)) {
            ...
            if (wtype == QuteWidgetType::CONTROLLER) {
                ...
                if (!channel2.isEmpty()) {
                    ...
                    if(!seenChannelNames.contains(channel2)) {
    ...
    ud->outputChannelNamesSet = seenChannelNames;
...
void CsoundEngine::logWidgetMessage(QString message, int role)
{
    // Widget messages (e.g. unknown property) originate on the GUI thread, so
    // they can be delivered straight to the registered consoles. This keeps the
    // explicit role (colour) which the Csound message queue does not carry.
    for (int i = 0; i < consoles.size(); i++) {
        consoles[i]->appendMessage(message, role);
    }
    if (ud->wl) {
        ud->wl->appendMessage(message, role);
    }
}

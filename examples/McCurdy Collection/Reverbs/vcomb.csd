...
instr	2
	...
	aresR 	vcomb	gasigR,     gkrvt,         gklpt,                  .1 
			outs		aresL * gkamp, aresR * gkamp									;SEND vcomb OUTPUT TO THE AUDIO OUTPUTS AND SCALE USING THE GUI SLIDER VARIABLE gkamp
...
<bsbPanel>
 ...
 <bsbObject version="2" type="BSBLabel">
  ...
  <midicc>0</midicc>
  <label>--------------------------------------------------------------------------------------------------
Vcomb implements a comb filtering effect on an audio input. This is described in the manual as 'coloured reverb'. In effect the comb filter is simply a delay with a feedback loop.
Delay time is expressed as 'Loop Time' in the context of this opcode but it is effectively the same thing as delay time. A ratio for the amount of feedback is not directly accessible but instead the user defines a value for 'Reverb Time' which is the time it will take for an impulse's reverberant tail to decay to 1/1000 of its initial strength (a 60 dB drop). The use of feedback can create dramatic rises in output amplitude, therefore in this example I have included a slider that controls output amplitude. Vcomb produces an audible transformation similar to the 'wguide1' and 'streson' opcodes but as it lacks any lowpass filtering within its algorithm it produces a sharper sound. Vcomb differs from the 'comb' opcode in that it allows the user to vary the 'Loop Time' parameter at rates other than i-rate. Because of this vcomb takes an additional input argument in which the user must declare the maximum 'Loop Time that the opcode should expect (for memory allocation purposes). In this example the user can choose between a sound file input or the computer's live input. An input gain control on the live input is also provided.</label>
  <alignment>left</alignment>
  <font>Liberation Sans</font>

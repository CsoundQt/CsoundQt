...
Here it contains an arrangement of checkboxes and a controller-window.

The controller window is a two dimensional fader, so it sends values to two different channels.

Checkboxes work similar to buttons, but keep their last state. They fit very well, to turn on/off parts of an instrument. 
Therefor the checkbox condition needs to be verified, while the instrument runs. This is made with the "if .. then .. endif" construct.
The "if" part checks the condition. If it is <true>, then the "then" part will be executed, if it is <false>, it jumps directly behind endif.
...
ioMeter {36, 168} {318, 253} {0, 59904, 0} "point_x" 0.559841 "point_y" 0.584125 point 7 0 mouse
ioText {37, 2} {134, 30} label 0.000000 0.00100 "" left "Lucida Grande" 16 {0, 0, 0} {65280, 65280, 65280} nobackground noborder 2. Run Csound
...
ioText {36, 36} {514, 39} label 0.000000 0.00100 "" left "Lucida Grande" 16 {0, 0, 0} {65280, 65280, 65280} nobackground noborder 3. Press one of these three buttons and move the point.
ioButton {179, 2} {100, 30} value 1.000000 "_Play" "Run Csound" "/" i1 0 10
ioCheckbox {36, 73} {20, 20} off read
